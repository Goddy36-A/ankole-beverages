...
from app.models.models import (
    Sale, SaleItem, Purchase, PurchaseItem, Product, Customer,
...
def sales_report():
    ...
    if export == 'csv':
        return _csv_export(
            ['Invoice', 'Date', 'Customer', 'Type', 'Subtotal', 'Discount', 'Tax', 'Total', 'Paid', 'Balance'],
            [[s.invoice_number, s.created_at.date(), s.customer.name, s.sale_type,
              s.subtotal, s.discount, s.tax, s.total_amount, s.amount_paid, s.balance]
             for s in sales],
            'sales_report.csv'
        )

    return render_template('reports/sales_report.html',
        sales=sales, total_revenue=total_revenue, total_paid=total_paid,
        total_balance=total_balance, customers=customers,
        from_str=from_str, to_str=to_str, customer_id=customer_id)
...
def inventory_report():
    ...
    if export == 'csv':
        return _csv_export(
            ['SKU', 'Product', 'Category', 'Stock', 'Reorder Level', 'Cost Price', 'Stock Value', 'Status'],
            [[p.sku, p.name, p.category.name, p.current_stock, p.reorder_level,
              p.cost_price, p.stock_value, 'Active' if p.is_active else 'Inactive']
             for p in products],
            'inventory_report.csv'
        )

    return render_template('reports/inventory_report.html',
        products=products, total_value=total_value, low_only=low_only)
...
def purchase_report():
    ...
    q = Purchase.query.filter(Purchase.created_at >= from_dt, Purchase.created_at <= to_dt)
    if supplier_id:
    ...
    if export == 'csv':
        return _csv_export(
            ...
            [[p.purchase_number, p.created_at.date(), p.supplier.name, p.invoice_number or '', p.status, p.total_amount]
             for p in purchases],
            'purchase_report.csv'
        )

    return render_template('reports/purchase_report.html',
        purchases=purchases, suppliers=suppliers, total=total,
        from_str=from_str, to_str=to_str, supplier_id=supplier_id)
...
def customer_balances():
    ...
    rows = []
    for c in customers:
        invoices = Sale.query.filter(Sale.customer_id == c.id, Sale.balance > 0).all()
        overdue = sum(s.balance for s in invoices if s.due_date and s.due_date < now)
        if overdue_only and overdue == 0:
            continue
        rows.append({'customer': c, 'overdue': overdue})
    return render_template('reports/customer_balances.html', rows=rows, overdue_only=overdue_only)
...
def movements_report():
    ...
    movement_types = ['PURCHASE', 'SALE', 'SALE_RETURN', 'PURCHASE_RETURN',
                      'ADJUSTMENT_IN', 'ADJUSTMENT_OUT', 'DAMAGE', 'EXPIRY', 'LOSS']
    return render_template('reports/movements_report.html',
        movements=movements, movement_types=movement_types,
        from_str=from_str, to_str=to_str, mv_type=mv_type)
